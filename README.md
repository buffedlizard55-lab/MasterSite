...
| Metric | Value |
|---|---|
| GitHub Pages sites published in the directory | **63** |
...
For each of the **61 GitHub Pages sites** in the directory:
...
- **GitHub Pages Status & Build Source** — 60 reporting `built` and 1 (`Elections`) reporting `errored`, recorded as-is rather than assumed healthy (`IRR-83`); 60 publishing from `main /` and 1 (`GOLD`) from `main /docs`
...
- **Per-entry audit flags** and an **87-entry flagged irregularities register** (`IRR-01` … `IRR-87`), ordered by severity
...
Ordered by what actually threatens the project's core promise. **Items 1 and 2 are the work for the next session, and item 1 is blocked only on credentials.**

| # | Task | Why it matters | Effort |
|---|---|---|---|
| 1 | **Once GitHub credentials are restored, re-run the full toolchain: `python3 tools/build_data.py` (a real refresh, not `--overlay-only`), then all three verifiers.** | This pass's API snapshot is from `2026-09-22T23:32:49Z` and its three verifiers were **never run** — recorded as blocked, not passed (`IRR-85`). A full build is also the only thing that can list `PRICINGEXPERT`, replace the now-false reason in `counts.unlisted`, re-derive every `kind` against the live `index.html` probe, and resolve the 21-entry queue's snapshot heads (`IRR-86`). | **Low effort, hard blocker — start here** |
...
| 4 | **Fix or explain `Elections`' failing Pages build** (`IRR-83`): `/pages` reports `errored`, build `1232488579` reads `Page build failed.`, and it persisted through four automatic re-reads. | The only non-`built` entry of 63. GitHub keeps serving the last successful build, so the live URL and the repository state have quietly diverged — exactly the kind of thing this directory exists to surface, and only the owner can resolve it. | Owner / upstream |
