...
The **master list and directory of the GitHub Pages sites published under** [`buffedlizard55-lab`](https://github.com/buffedlizard55-lab) (with a verified audit of [`kanlerxz87-cyber`](https://github.com/kanlerxz87-cyber)) — generated entirely from official GitHub REST API reads, with zero manual data entry and zero hallucinations. One repository is deliberately unpublished by owner request, and two sites that were listed in earlier passes are frozen as unreachable because they no longer exist upstream.

**Live Directory:** <https://buffedlizard55-lab.github.io/MasterSite/>

---

## Audit status — 2026-09-24 (API snapshot `2026-09-24T00:39:08Z`; **all four read-only tools run against that snapshot**)

| Metric | Value |
|---|---|
| GitHub Pages sites published in the directory | **69** |
| Interactive apps / documentation stubs | **60 / 9** — the last two stubs are `MALTA2` and `NOBEL-PRIZE`, both empty placeholders created *during* this pass and listed rather than left out, so the directory has no silent gap |
| Pages builds reporting `built` | **69 / 69** — one transient `building` state (`THUNDERPICK-WC-2026`) was recorded as-is and then folded in by re-running the generator, exactly as `AGENTS.md` prescribes; `verify_live.py` reported it as the pass's only hard mismatch and the re-run cleared it |
| Descriptions stamped with the commit they were read at | **69 / 69** (`verifiedAtSha`) — and the two entries that previously held a non-SHA were resolved (`IRR-108`) |
| Descriptions provably behind their repository head *at the snapshot* | **23** — the published work queue, rendered stale-first in `VERIFICATION.md` § 4a. This figure was previously meaningless for two entries: `KalshiPaperSim` was stamped `AUTO:COUNTS` and `MasterSite` was stamped `main`, so neither could ever equal a commit SHA and both reported stale on every build forever (`IRR-108`). Both now carry real SHAs |
| Descriptions re-read in this pass / carried forward | **8 / 61** — the six newly published repositories were read in full at head with their own suites executed, and `KalshiPaperSim` and `MasterSite` were rewritten because their stamps were not SHAs |
| Verifier results | **RUN against the shipping snapshot.** `verify_live.py`: 894 checks, 893 ok, **0 hard mismatches after the prescribed re-run** (the single hard mismatch was the transient `building` state above). `audit_kind.py`: **68 checked, 0 disagreements**. `audit_descriptions.py` after `rm -rf tools/.readme-cache`: **38 accepted exceptions, 0 entries needing manual confirmation**. The Playwright contract still runs in CI only — no browser binary or CDN access exists here, so no local run is claimed |
| Commits audited across the listed repositories | **3761**, and 3,761 is only true at the moment it was read: this pass watched it move 3,629 → 3,793 → 3,753 → 3,758 → 3,761 across six builds of one pass. and the figure moves every few minutes while 69 repositories are actively pushed to |
| Public repositories on the account (API) | **70** |
| Pages sites on the account (API) | **70** |
| Repositories withheld, each with a stated reason | **1** (`ProjX` — permanently excluded by owner request). `listed + withheld = 69 + 1 = 70` is asserted at build time |
| Unreachable entries (needs owner review) | **2** (`JobSearchSF` — HTTP 404 for an eighth consecutive audit; `MALTA` — deleted mid-pass, `IRR-109`) |
| Irregularities registered | **109** (`IRR-01` … `IRR-109`) — five registered this pass (`IRR-105` … `IRR-109`), of which 2 critical and 3 warn |
| Accepted description exceptions / description notes | **38 / 18** |
| Categories | **11** — Sports Data & Scoreboards (26), Travel & Korea Trip (11), Markets & Trading Research (11), SF Local Guides (7), Travel & Event Dossiers (2), Science & ML Research (3), Directory & Meta (3), Gaming & Guides (2), Elections & Civic Data (1), Health & Personal Guides (1), Social & Creator Data (1) |
| Pages publish sources | **67** from `main /`, **1** (`GOLD`) from `main /docs` |

`Travel & Event Dossiers` is a new category created by this pass for the Malta/Thunderpick travel work, and the UI needed no change to support it: the filter chips are built from the data, and `AGENTS.md` forbids hardcoding a category list.
...
For each of the **69 GitHub Pages sites** in the directory:
...
- **GitHub Pages Status & Build Source** — **69 / 69 reporting `built`** at the shipping snapshot, each recorded as-is rather than assumed healthy, and any non-`built` state published rather than hidden; 67 publishing from `main /` and 1 (`GOLD`) from `main /docs`
...
- **Per-entry audit flags** and a **109-entry flagged irregularities register** (`IRR-01` … `IRR-109`), ordered by severity
...
| [`data/sites.js`](data/sites.js) | **Generated** verified dataset — 61 sites, 1 unreachable entry, 87 irregularities, 2 audited accounts, methodology |
...
1. **The prose is the fragile part, not the numbers — and 2026-09-22 proved it three more times.** Every timestamp, SHA, commit count and Pages status is read from `api.github.com`; the last clean re-check was the 2026-09-21 pass (686/686 live field checks, 0 hard mismatches), and **this pass could not re-run it at all** (`IRR-85`). Descriptions are written prose, and this pass found **two published descriptions that had gone false** — `SelfLearn` and `MasterSelfLearn`, both published as empty one-file placeholders and now research engines of 234 and 111 files (`IRR-84`) — plus **one entry caught before it was ever published**: `NFLPARLAYCOMP`'s headline PnL flipped from **+135.04 to −92,876.56** in the 40 minutes between read and write (`IRR-82`). That is the same class as `NFLComp`'s +$2.43M-that-was-−$353K (`IRR-65`), reached without shipping the wrong number. `verifiedAtSha` reliably tells us *which* entries to re-read; it cannot tell us what they now say.
...
9. **`kanlerxz87-cyber` contributes nothing.** Verified to exist (`type: User`, created `2026-08-03T20:56:16Z`) with **0** public repositories (re-verified `2026-09-22T23:32:49Z`). If it ever publishes, the generator picks it up automatically.
10. **Two repositories are withheld, for two different reasons, and the difference matters.** `ProjX` is **permanently excluded by owner request**, so the directory will always show one fewer site than the account totals — deliberate, and never to be "fixed"; see [Repository Exclusions](#repository-exclusions--standing-instructions-for-future-sessions). `PRICINGEXPERT` is withheld only **procedurally**: it is fully curated in `tools/overlay.json` and cannot be listed until a full `build_data.py` run, because the offline render cannot add a site (`IRR-86`). Its stale reason string in `counts.unlisted` is visible in the published data until then.
11. **`JobSearchSF` cannot be resolved from here** — HTTP 404 for a fifth consecutive audit. Owner decision only (`IRR-01`).
...
| 2 | **Clear the 21-entry `PROSE-STALE` queue by re-reading each description against its repository — and open the *data files*, not just the README.** The queue is printed by every build and rendered stale-first in `VERIFICATION.md` §4a. | Still the only error class that has materially misled a reader, now across six consecutive audits; this pass produced a headline PnL that changed **sign** mid-audit (`IRR-82`). `verifiedAtSha` turns an unbounded job into a work queue but cannot do the reading, and `IRR-70` shows a README-unchanged diff is not a safe shortcut. Two entries have durable triage rules in their `verifiedBasis` already (`SFWeather`, `MasterSelfLearn`) — use them instead of re-deriving. | **High — the real work** |
| 3 | **Run the interface test suite in an environment that has a browser.** Four tests were added this pass for the density preference and the stacked narrow-screen table; none could be executed here, because `cdn.playwright.dev` is unreachable and no system browser exists. The DOM contract *was* verified offline in jsdom (39 checks), but layout — `scrollWidth ≤ innerWidth` at 375×812, touch-target heights — is only observable in a real browser. | The published site's readability on a phone is a claim this repository cannot currently evidence from the sandbox. `.github/workflows/test.yml` runs all four projects, including `mobile-chromium`. | Low — CI does it on push |
| 4 | **Resolve the two repositories deleted or emptied upstream: `MALTA` (`IRR-109`) and `JobSearchSF` (`IRR-01`).** | `MALTA` was fully verified in this pass — 40 commits, four clean checks — and then vanished, taking the work with it; `MALTA2`, created three minutes later, holds an 8-byte README and no `index.html`. A directory cannot distinguish "deliberately replaced" from "accidentally deleted", and only the owner can. | Owner only |
| 5 | **Report the three self-contradiction defects upstream** (`Elections` 209/189/229 and 63/58, `NBAInjuryReport` 65/21 vs 94/70, `OLBG-Competition` 262/258) so the repositories fix their own prose instead of this directory carrying a permanent footnote. | A directory that grades other projects' verification should not need a standing workaround for their arithmetic. Each is a one-line fix at the source. | Owner / upstream |
| 6 | **Resolve `JobSearchSF` (`IRR-01`).** Restore the repository, or confirm it is retired so the frozen entry can be dropped. | The oldest open `critical` irregularity (2026-09-17), now in its fifth audit. No machine can resolve it. | Owner only |
| 7 | **Fetch live pages over HTTP from an environment with egress to `*.github.io`.** A GitHub Actions job in this repository would have that access even though the audit sandbox does not. | It is the difference between "GitHub says it built" and "it works" — and `IRR-83` is precisely a case where the two disagree. The blocker is the sandbox, not the design. | **Medium — highest-value new capability** |
...
| 9 | **Decide whether overlapping entries should stay listed.** `HotelSeoulRoughdraft1` duplicates `Itinerary-Korea` (`IRR-14`); `MLB-PBP`/`MLB-Live-PBP` overlap, and `MLBRainDelay` is now explicitly built on `MLB-Live-PBP`'s design; `ScheduleFreeTime` and `VacationSchedule` overlap without naming each other (`IRR-48`). The new `*ENGY` and `*Comp` families widen this: `NFLPARLAYCOMP` and `ParlaySports` both cover NFL parlay paper-trading, and `SelfLearn` / `MasterSelfLearn` are two engines over the same claim graph without cross-referencing. | A directory is more useful when it does not send readers to a superseded draft, and 63 repositories is past the point where a reader can tell the relationships out themselves. Curation, not data. | Owner only |
| 10 | **Automate the audit on a schedule** (GitHub Actions running `build_data.py` + all three verifiers, opening a PR when the gate is non-zero). | Ten repositories appeared in 23 hours and four changed state *during* this one audit. A scheduled job would keep the window between snapshot and reality to hours instead of days — and, unlike this sandbox, it would hold working credentials and reach the live sites (items 1, 3 and 7). | Medium |

| 11 | **Report the three new cross-repository findings upstream** (`IRR-105` Malta-family outage contradiction, `IRR-106` `InjuryAlerTNFL`'s "zero schedule events", `IRR-107` `KalshiPaperSim`'s README-versus-artifact divergence and its 0-to-2 in-play reversal). | Each is a one-line correction in the repository that owns it, and `IRR-107` is the sharpest: a project whose central honesty claim is "0 in-play ladders, reported as unverified rather than as in-play" now records 2, and its README still says 0. A directory that grades other projects' verification should not have to carry that as a permanent footnote. | Owner / upstream |
| 12 | **Decide how to handle repository renames and deletions mid-pass as a standing rule.** | This pass lost 40 commits of verified work to a deletion that happened between two builds 6 minutes apart, and no earlier pass had hit the case where a *newly discovered* repository disappeared before it was ever published. The freeze-into-`retired[]` policy covered it, but the trigger — "was it ever published?" — had to be reasoned out in the moment. | Low — write the rule into `AGENTS.md` |
| 13 | **Fetch every Malta-family page's live state once egress is available, and settle `IRR-105`.** | Three repositories describe the same host's availability that day in three different ways, and the DAB/talkSPORT identity question underneath it ("which feed does Malta's 6C multiplex carry") is answerable by one receiver test or one email. From this sandbox, `curl` to anything except `api.github.com` returns HTTP `000`. | **Medium — blocked on egress, not effort** |

**Closed this pass:**

- ~~**Describe `SelfLearn` and `MasterSelfLearn` properly once they contain something.**~~ Both were rewritten from clones with their test suites executed (`IRR-84`), and both moved `stub` → `app`. The two stub entries that existed only to keep the accounting honest are gone.
- ~~**`--overlay-only` silently republishes a `kind` the auditor has already disproved.**~~ `kind` is overlay-sourced in `main()`, so it is now rendered by the offline path too, with `counts.apps` / `counts.stubs` recomputed beside it (`IRR-86`).
- ~~**A pass that stamps as it goes under-reports its own work.**~~ `counts.proseReReadLatestPass` and the UI's "Prose re-read / Prose carried" badges now agree with what actually happened, and the durable rule is recorded so the next pass stamps once (`IRR-87`).
- ~~**The table could not show more than ~13 entries per screen and was unreadable on a phone.**~~ Compact is the default, a Density toggle restores the padded layout per browser, and below 900px the nine columns re-stack into labelled lines with the DOM unchanged.
